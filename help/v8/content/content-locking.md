...
Adobe Campaign允许您通过锁定整个模板或特定结构和组件来锁定电子邮件模板中的内容。 这样可防止无意中编辑或删除，从而更好地控制模板自定义，并提高电子邮件促销活动的效率和可靠性。
<!--
>[!IMPORTANT]
>
...
可以在&#x200B;**结构**&#x200B;级别或&#x200B;**组件**&#x200B;级别应用内容锁定。 下面是在锁定模板中的内容时在结构和组件级别应用的主要原则：

* 锁定结构时：

   * 默认情况下，该结构中的所有内容也将被锁定。
   * 无法向结构添加任何内容。
...
* 当结构可编辑时（结构未锁定）：
   ...
   * 单个内容组件可以锁定在该结构中。
   * 默认情况下，如果组件已锁定或选择了“仅限可编辑的内容锁定”，则无法删除该组件。 您可以通过启用“允许删除”选项来覆盖此限制。
...
1. 如果您选择了&#x200B;**[!UICONTROL 内容锁定]**&#x200B;模式，则可以进一步定义用户与模板的交互方式。 打开&#x200B;**[!UICONTROL 启用内容添加]**&#x200B;选项，然后选择以下选项之一：

   * **[!UICONTROL 允许添加结构和内容]**：用户可以在现有结构和内容之间添加结构，并在可编辑结构中添加内容组件或片段。

   * **[!UICONTROL 仅允许添加内容]**：用户可以在可编辑结构中添加内容组件或片段，但无法添加或复制结构。
...
1. 选择锁定模式后，您可以定义在选择了&#x200B;**[!UICONTROL 内容锁定]**&#x200B;模式时要锁定的结构和/或组件：
   ...
   * [了解如何锁定组件](#lock-components)
   ...
   如果您选择&#x200B;**[!UICONTROL 只读]**&#x200B;模式，请继续像往常一样完成并保存模板。

在设计模板时，您可以通过选择模板正文，随时调整&#x200B;**[!UICONTROL 管理]**&#x200B;设置。 为此，请单击位于右侧窗格顶部的导航边栏中的&#x200B;**[!UICONTROL 正文]**&#x200B;链接。
...
锁定结构后，不能再在其中复制或添加其他内容组件或片段。 默认情况下，锁定结构中的所有元件也将被锁定。 要使组件在锁定结构中可编辑，请执行以下操作：
...
在以下示例中，为模板正文启用了管理。 *结构2*&#x200B;被锁定，其中&#x200B;*组件1*&#x200B;可编辑，而&#x200B;*结构3*&#x200B;被完全锁定。
