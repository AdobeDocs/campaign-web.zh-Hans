...
Adobe Campaign v8 是新一代营销活动工具，专为各种营销渠道（如电子邮件、推送通知、短信和直邮）构建。它提供了强大的 ETL 和数据管理功能，以帮助制定和策划完美的营销活动。其编排引擎支持丰富的多接触点营销计划，核心重点是批量驱动的历程。它还包括一个可扩展的实时消息服务器，允许营销团队根据来自任何 IT 系统的全面负载发送预定义的消息，用于密码重置、订单确认、电子收据等多种通信场景。

使用 Campaign 可以：

* 通过单一的客户访问视图，**促进**&#x200B;个性化和参与度。
* 将电子邮件、移动设备、线上和线下渠道&#x200B;**整合**&#x200B;到客户历程中。
* **自动**&#x200B;投放有意义、及时的消息和优惠。

## 探索 Campaign Web 用户界面 {#web}

最初只能通过功能丰富的[客户端控制台](#ac-client)使用 Campaign，而它现在提供一个新的 Web 用户界面 (UI)，该界面提高了可用性、可访问性并采用新设计以显著改善您的用户体验。这一现代 UI 简化了营销活动的设计和投放，并实现了与其他 Adobe 解决方案（包括 Adobe Experience Platform）的一致性。

![Adobe Campaign Web 用户界面主屏幕的屏幕快照](assets/home.png){zoomable="yes"}

这款新的 Web 用户界面主要满足&#x200B;**商业从业者**&#x200B;的需求。此初始版本中暂不支持典型的管理任务，但后续版本将予以解决。请注意，并非客户端控制台中的所有功能或选项目前都可以在新 UI 中使用。未来版本中将推出新的用例、选项和功能。

作为管理员或专家用户，如果您需要访问 Campaign Web 用户界面中未提供的 Campaign v8 功能，请连接到[客户端控制台](#ac-client)。
...
Campaign 数据存储在应用程序服务器中。数据既可以从客户端控制台访问，也可以从 Campaign Web 用户界面访问。例如，如果您使用客户端控制台创建投放模板，则它在 Campaign Web 用户界面中也可用。同样地，如果您在 Campaign Web 用户界面中创建电子邮件投放，则也可从客户端控制台访问此投放。

一些对象只能在客户端控制台中进行创建和管理。这些对象可以在 Campaign Web 用户界面中查看和使用，但无法在此环境中创建或修改。所有 Campaign 对象和组件均可从“资源管理器”视图中获取，该视图可通过左侧导航栏进入。
...
>[!VIDEO](https://video.tv.adobe.com/v/3453432?captions=chi_hans&quality=12)
-->
