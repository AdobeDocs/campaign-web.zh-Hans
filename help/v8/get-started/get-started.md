...
Adobe Campaign提供了跨渠道客户体验设计平台，并为可视化的活动编排、实时互动管理和跨渠道执行提供了环境。

Adobe Campaign v8是新一代营销活动工具，专为各种营销渠道（如电子邮件、推送通知、短信和直邮）构建。 它提供了强大的 ETL 和数据管理功能，以帮助制定和策划完美的营销活动。其编排引擎支持丰富的多接触点营销计划，核心重点放在批量驱动历程。 它还包含一个可扩展的实时消息服务器，允许营销团队根据来自任何IT系统的综合有效负载发送预定义的消息以进行通信，例如密码重置、订单确认、电子收据等。

使用 Campaign 可以：

* **通过客户的单个可访问视图推动**&#x200B;个性化和参与。
* **将**&#x200B;电子邮件、移动设备、线上和线下渠道整合到客户历程中。
* **自动投放**&#x200B;有意义、及时的消息和优惠。

## 探索 Campaign Web 用户界面 {#web}

Campaign最初只能通过丰富的[客户端控制台](#ac-client)使用，现在它提供了新的Web用户界面(UI)，增强了可用性、可访问性，以及可显着改善用户体验的新设计。 这个现代化的UI简化了营销活动的设计和交付，并与其他Adobe解决方案(包括Adobe Experience Platform)保持一致。

![Adobe Campaign Web用户界面主屏幕截图](assets/home.png){zoomable="yes"}

此新Web用户界面主要满足&#x200B;**业务从业者**&#x200B;的需求。 典型的管理任务在此初始版本中不可用，但将在后续版本中解决。 请注意，并非客户端控制台中可用的所有功能或选项当前都可在新UI中访问。 未来版本中将引入新的用例、选项和功能。

作为管理员或专家用户，如果您需要访问Campaign Web用户界面中不提供的Campaign v8功能，请连接到[客户端控制台](#ac-client)。
...
客户端控制台专为管理员和高级用户执行其任务而设计。Campaign客户端控制台是安装在您系统上的基于浏览器的应用程序。 它通过 Web 服务 API 连接到您的 Campaign 应用程序服务器。

Campaign 数据存储在应用程序服务器中。可从客户端控制台和Campaign Web用户界面访问数据。 例如，如果您使用客户端控制台创建投放模板，则Campaign Web用户界面中也会提供该模板。 同样，如果您在Campaign Web用户界面中创建电子邮件投放，则可以从客户端控制台访问此投放。

一些对象只能在客户端控制台中进行创建和管理。这些对象可以在Campaign Web用户界面中查看和使用，但无法从此环境创建或修改。 所有Campaign对象和组件均可从资源管理器视图中访问，可从左侧导航中访问。

有关如何将Campaign v8与其客户端控制台一起使用的更多信息，请参阅[Campaign v8 （客户端控制台）文档](https://experienceleague.adobe.com/docs/campaign/campaign-v8/campaign-home.html?lang=zh-Hans){target="_blank"}。
