...
浏览左侧的链接以访问Campaign v8 Web功能。 多个链接显示可排序和过滤的对象列表。 您还可以配置列以显示所需的所有信息。 请参阅 [部分](#list-screens). 所有列表屏幕都是只读的，但电子邮件投放列表除外。Alpha中不提供单击任何用于编辑/查看的列表项。 所有列表都将在将来的版本中可编辑。 左侧导航菜单中显示的项目取决于您的用户权限。
...
>title="活动"
>abstract="这是您的营销活动列表。其中可显示有用的信息，例如它们的开始/结束/最后修改日期，以及它们的状态。您可以按状态或开始/结束日期过滤列表。也会提供营销活动模板。这些列表为只读状态。"

>[!CONTEXTUALHELP]
>id="acw_deliveries_list"
>title="投放"
>abstract="浏览您的投放列表。您可以查看它们的状态、最后修改日期以及关键 KPI。您可以按状态、联系日期或渠道过滤列表。单击一封要投放的电子邮件以打开其仪表板。其他项目为只读状态。也会提供投放模板。"

* **促销活动**  — 这是营销活动列表。 默认情况下，您可以查看其开始/结束/上次修改日期及其状态。 您可以按状态或开始/结束日期过滤列表。也会提供营销活动模板。这些列表为只读状态。
...
>title="受众"
>abstract="这是您的受众列表。您可以查看它们的类型、来源、创建/最后修改日期和标签。您可以按来源过滤列表。此列表是只读的。"
...
* **订阅列表**  — 浏览订阅列表。 默认情况下，您可以查看其类型、模式和标签。 此列表是只读的。
* **定位工作流**  — 访问Campaign工作流的列表。 默认情况下，您可以查看其状态、上次/下次处理日期和环境。 您可以按状态、最后处理日期和工作流类型过滤列表。也会提供工作流模板。这些列表为只读状态。
...
* **选件**  — 浏览您的互动选件列表。 默认情况下，您可以查看它们的状态、开始/结束日期和环境。您可以按状态和开始/结束日期过滤列表。也会提供优惠模板。这些列表为只读状态。
