...
浏览至主页以检查您平台的关键绩效指标。这些指标显示已投放、已打开、已点击、退订的消息的数量和百分比以及错误率。
...
>title="已投放"
>abstract="对于所选渠道，此指标显示已成功处理的所有消息的总数，以及已成功投放消息数与已发送消息总数的百分比。"

已投放消息数反映了您的可投放性率。出于以下原因，它绝不可能为 100%：某些地址或电话号码错误、电子邮件提供商的垃圾邮件拦截程序可能拒绝您的消息或者发生可投放性问题。
...
“种子地址”字段等于“否”且状态等于“由服务提供商考虑”（对于短信）、“已发送”（对于电子邮件）或“在移动设备上收到”的邮件计数（对于推送通知）的消息的计数。
...
>title="打开次数"
>abstract="对于所选渠道，此指标显示已打开的所有消息的总数，以及已打开消息数与已成功投放消息总数的百分比。"

打开总数是通过跟踪消息的打开总次数来计算的，无论打开消息的收件人的数量如何。此指标仅适用于电子邮件。
...
>title="点击次数"
>abstract="对于所选渠道，此指标显示消息中点击的所有 URL 的总数，以及点击次数与已成功投放消息总数的百分比。"
...
此指标基于整合跟踪表 (`nms:trackingStats`)。出于性能原因，在显示报告时将使用此聚合表来代替收件人跟踪日志表 (`nms:trackingLogRcp`)，并且它不是实时计算的。该表是在检索跟踪日志几分钟后生成的。
...
>title="Campaign中的投放模板"
>abstract="待确认"

>[!CONTEXTUALHELP]
>id="acw_workflow_template_for_campaign"
>title="Campaign中的工作流模板"
>abstract="待确认"
