...
>title="保存过滤器"
>abstract="选择一个现有的预定义过滤器。 预定义过滤器是用户创建并保存的自定义过滤器，以供将来使用。 它们可在使用查询建模器进行过滤操作时充当快捷方式。 例如，它们可以在过滤数据列表或创建投放的受众时使用。"
...
>title="保存过滤器"
>abstract="在此界面，您可以将规则保存为预定义过滤器。 您可以创建新的预定义过滤器，或覆盖现有过滤器。 如需将过滤器保存为收藏，请开启&#x200B;**[!UICONTROL 收藏过滤器]**&#x200B;选项。 您也可以启用&#x200B;**[!UICONTROL 共享过滤器]**&#x200B;选项，让其他用户也能使用该过滤器。"
...
1. 选择&#x200B;**创建新过滤器**，然后输入该过滤器的名称和说明。
   ...
   如果需要，可将过滤器保存为收藏项目。 可在[此部分](#fav-filter)中了解详情。 您也可以启用&#x200B;**[!UICONTROL 共享过滤器]**&#x200B;选项，让其他用户也能使用该过滤器。 可在[此部分](#share-filter)中了解详情。
...
共享过滤器会显示在所有用户的预定义过滤器列表中，允许他们在自己的工作流和查询中使用这些过滤器。
...
1. 浏览到用于创建受众的预定义过滤器，选择它并确认。
   ...
   您也可以使用![](../content/assets/do-not-localize/Smock_More_18_N.svg)按钮来编辑或删除现有的预定义过滤器。
...
您还可以直接从规则生成器编辑或删除预定义过滤器。 在预定义过滤器选择窗口中，使用&#x200B;**更多操作**&#x200B;按钮。 您可以删除或编辑过滤器。 您可以修改标签、添加描述、将其添加到收藏夹，或切换&#x200B;**[!UICONTROL 共享过滤器]**&#x200B;选项，以控制谁可以访问该过滤器。
