...
>title="预定义过滤器管理"
>abstract="Campaign Web 现在为您提供一个用户容易理解的界面，轻松地管理和自定义预定义过滤器以满足您的特定需求。创建一次并保存以供将来使用。"
...
预定义过滤器是创建和保存以供将来使用的自定义过滤器。 在使用规则生成器的任何过滤操作期间，例如，在过滤数据列表或创建投放的受众时，可以将它们用作快捷键。
...
1. 选择 **创建新筛选器**，并输入该过滤器的名称和描述。
   ...
   如果需要，可将过滤器另存为收藏夹。 可在[此部分](#fav-filter)中了解详情。
...
您可以在构建规则时将预定义过滤器添加为收藏，以便快速访问。 可在[此部分](#fav-filter)中了解详情。
