...
1. 展开该复选框（如果有），或者使用 **[!UICONTROL 添加]** 按钮。 选择该复选框应更新数据库中的服务或字段。
   ...
   如果您选择 **[!UICONTROL 订阅和服务]**，选择一个 [服务](../audience/manage-services.md) 从列表中选择以下两个选项：
...
1. 添加或更新所有字段后，单击 **[!UICONTROL 行动号召]** 以展开相应的部分。 它允许您在中定义按钮的行为 **[!UICONTROL 表单]** 组件。 [了解如何操作](#define-actions-on-form-submission)
...
1. 定义单击按钮时将发生的操作：
   ...
   * **[!UICONTROL 确认页面]**：默认情况下，用户将被重定向到 **[!UICONTROL 确认]** 页面集。

   * **[!UICONTROL 重定向URL]**：输入用户将被重定向到的页面的URL。

   * **[!UICONTROL 登陆页面]**：您可以为用户选择另一个要重定向到的登陆页面。 确保相应地配置选定的登陆页面。

1. 如果要在提交表单时进行其他更新，请选择 **[!UICONTROL 其他更新]**，然后选择要更新的项目：
   * 订阅 [服务](../audience/manage-services.md)  — 在这种情况下，定义您要在提交表单时选择加入还是选择退出用户。 设计电子邮件时，如果定义了 **[!UICONTROL 登陆页面]**-type链接至此登陆页面，所选服务将被自动使用。 [了解有关插入链接的更多信息](../email/message-tracking.md)

     >[!NOTE]
     >
     >如果要在此登陆页面中使用多项服务，请使用 **[!UICONTROL 来自URL的服务]** 选项中所述的。
...
1. 选择 **[!UICONTROL 来自URL的服务]** 选项允许将登陆页面用于多项服务，使其成为动态页面。 定义在提交表单时要选择加入还是选择退出用户。
   ...
   设计电子邮件时，如果定义了 **[!UICONTROL 登陆页面]**-type链接至此登陆页面，您将能够从列表中选择任何服务。 然后，在定义指向此登陆页面的其他链接时，您可以选择其他服务。 [了解有关插入链接的更多信息](../email/message-tracking.md)
