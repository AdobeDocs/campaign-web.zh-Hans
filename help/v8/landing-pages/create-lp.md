...
>title="定义预加载选项"
>abstract="在选定&#x200B;**预先填写表格中引用的数据**&#x200B;选项后，如果登陆页面的访客与数据库中的配置文件匹配，则自动在表单中预加载配置文件的信息。如果选定&#x200B;**如果没有 ID，则跳过预加载**&#x200B;选项，则输入的每个配置文件将在审批表单后添加到数据库中。"

>[!CONTEXTUALHELP]
>id="acw_landingpages_storage"
>title="定义存储选项"
>abstract="预加载部分可让您指示如何在数据库中查找要更新的记录。"
...
1. 在 **[!UICONTROL 数据预加载]** 部分，默认情况下会选中以下两个选项：
   ...
   * 在选定&#x200B;**[!UICONTROL 预先填写表格中引用的数据]**&#x200B;选项后，如果登陆页面的访客与数据库中的配置文件匹配，则自动在表单中预加载配置文件的信息。用户只需填写缺少的字段，并在需要时更新现有值。 这允许合并现有用户档案的数据，而不是创建重复项。
