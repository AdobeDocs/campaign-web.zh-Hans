...
>title="条件"
>abstract="**测试**&#x200B;活动可以有多个输出过渡。在执行工作流期间，将会按顺序测试每个条件，直到满足其中一个条件。如果没有满足任何条件，工作流程会沿着&#x200B;**[!UICONTROL 默认条件]**&#x200B;路径继续。如果未激活默认条件，则工作流将在此时停止。"
...
1. 在执行工作流期间，将会按顺序测试每个条件，直到满足其中一个条件。如果没有满足任何条件，工作流程会沿着&#x200B;**[!UICONTROL 默认条件]**&#x200B;路径继续。如果未激活默认条件，则工作流将在此时停止。
