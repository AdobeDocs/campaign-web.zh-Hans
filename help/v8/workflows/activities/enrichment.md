...
**扩充**&#x200B;活动是一个&#x200B;**定位**&#x200B;活动。 它通过来自数据库的附加信息来增强目标数据。 它通常用于分段活动之后的工作流中。

>[!NOTE]
>
>**生成受众**&#x200B;活动（查询类型）还支持&#x200B;**扩充数据**。 查看[了解详情](build-audience.md#build-audience-configuration)。
...
例如，您可以将客户购买的相关信息添加到工作流表，并使用此数据将最新购买或购买金额的电子邮件个性化。
...
1. 添加&#x200B;**扩充**&#x200B;活动。
1. 如果在工作流中配置了多个过渡，请使用&#x200B;**[!UICONTROL 主集]**&#x200B;字段定义应将哪个过渡用作扩充的主集。
...
**扩充数据**&#x200B;节在&#x200B;**扩充**&#x200B;和&#x200B;**生成受众**（查询类型）活动中可用。 它可让您使用数据库中的附加信息（例如合同参考或新闻稿订阅）增强目标数据。 该数据将与受众一起存储在工作流&#x200B;**工作台**&#x200B;中，并且可用于后续的活动。 您可以添加单个扩充属性、集合链接或表达式，并访问高级选项。

单击&#x200B;**添加扩充数据**&#x200B;并选择要用于扩充的属性。 [了解如何选择属性并将其添加到收藏夹](../../get-started/attributes.md)。

您可以选择两种类型的扩充数据：从目标维度中选择单个扩充属性或收集链接。 以下示例详细介绍了每种类型：
...
>[!NOTE]
>
>属性选择屏幕中的&#x200B;**编辑表达式按钮**&#x200B;允许您构建高级表达式以选择属性。 [了解如何使用表达式编辑器](../../query/expression-editor.md)。

![显示扩充数据选择屏幕的屏幕快照](../assets/workflow-enrichment1.png)

添加至少一个扩充属性后，单击&#x200B;**[!UICONTROL 高级参数]**&#x200B;以配置扩充数据的生成方式，包括分组、重复数据删除、主键处理和入站事件数据。 这些选项反映了客户端控制台的镜像，并适用于高级工作流方案。

![显示扩充高级参数的屏幕截图](../assets/workflow-query-advanced-parameters.png)

>[!NOTE]
>
>**生成受众**&#x200B;和&#x200B;**扩充**&#x200B;活动之间的可用选项有所不同。

每个活动均可使用以下选项：

+++ 构建受众活动（查询类型）

* **[!UICONTROL 保留主集的所有附加数据]**：保留输出过渡中主集客集的附加列。
* **[!UICONTROL 按定向维度元素对数据进行分组]**：对结果进行分组，以使每个定向记录仅显示一次。
* **[!UICONTROL 删除重复行(DISTINCT)]**：从结果集中删除重复行。
* **[!UICONTROL 禁用自动添加定向维度的主键]**：阻止活动自动将定向维度主键添加到结果。
* **[!UICONTROL 禁用自动过滤0 ID记录]**：保留标识符值为0的记录，而不是自动过滤这些记录。
* **[!UICONTROL 使用入站事件数据]**：使用来自入站过渡的数据作为活动的工作输入。
...
* **[!UICONTROL 删除重复行(DISTINCT)]**：从结果集中删除重复行。
* **[!UICONTROL 禁用自动过滤0 ID记录]**：保留标识符值为0的记录，而不是自动过滤这些记录。
* **[!UICONTROL 为结果的每一行添加一个标识符]**：为每个输出行添加一个唯一标识符。
...
* **[!UICONTROL 1基数简单链接]**：来自主集的每个记录都与来自链接数据的一个及唯一一个记录相关联。
* **[!UICONTROL 0或1基数简单链接]**：主集中的每个记录都与链接数据中的0或1个记录相关联，但不与多个记录相关联。
* **[!UICONTROL N个基数集合链接]**：主集中的每个记录都与链接数据中的0、1个或多个(N)记录相关联。

要创建链接，请执行以下步骤：

1. 在&#x200B;**[!UICONTROL 链接定义]**&#x200B;部分中，单击&#x200B;**[!UICONTROL 添加链接]**&#x200B;按钮。

   ![显示链接定义部分的屏幕截图](../assets/workflow-enrichment-link.png)
...
>title="协调"
>abstract="**扩充**&#x200B;活动可用于协调来自 Campaign 数据库架构的数据与来自另一个架构的数据，或来自临时架构的数据（例如使用“加载”文件活动上传的数据）。 这种类型的链接定义了对唯一记录的协调。 Adobe Campaign 通过在目标表格中添加外键来创建指向目标表的链接，以存储对唯一记录的引用。"

**扩充**&#x200B;活动可用于协调来自 Campaign 数据库架构的数据与来自另一个架构的数据，或来自临时架构的数据（例如使用“加载”文件活动上传的数据）。 这种类型的链接定义了对唯一记录的协调。 Adobe Campaign 通过在目标表格中添加外键来创建指向目标表的链接，以存储对唯一记录的引用。
...
1. 确定要创建协调链接的数据。
   ...
   * 若要使用来自输入过渡的数据创建协调链接，请选择&#x200B;**临时架构**，然后选择存储目标数据的工作流过渡。
...
以下示例显示了一个工作流，该工作流配置为在Adobe Campaign数据库收件人表和生成&#x200B;**加载文件**&#x200B;活动的临时表之间创建链接。 在此示例中，扩充活动使用电子邮件地址作为协调条件来协调两个表。
...
下方的示例显示了一个工作流，该工作流配置为在两个过渡之间创建链接。 第一个过渡使用&#x200B;**查询**&#x200B;活动定位配置文件数据，而第二个过渡包括存储到通过加载文件活动加载的文件中的购买数据。
...
* 添加了第二个&#x200B;**扩充**&#x200B;活动，以便使用来自&#x200B;**加载文件**&#x200B;活动的购买数据扩充工作流表中的数据。 这样，我们便可以在进一步的活动中使用这些数据，例如，将发送给客户的消息与他们的购买信息个性化。
