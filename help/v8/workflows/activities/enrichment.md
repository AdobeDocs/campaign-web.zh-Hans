...
**扩充**&#x200B;活动是一个&#x200B;**定位**&#x200B;活动。它使用数据库中的附加信息增强目标数据。 它通常用于分段活动之后的工作流中。

浓缩数据可以来自以下任何一种：

* **与工作流程中定位的工作表相同的工作表**：
   * 定位一组客户，并向当前工作表中添加“出生日期”字段。
...
>title="扩充设置"
>abstract="强化设置：目标维度、其他属性和选项。"

1. 单击&#x200B;**添加扩展数据**，然后选择要用于扩展的属性。 [了解如何选择属性并将其添加到收藏夹](../../get-started/attributes.md)。

   您可以选择两种类型的强化数据：从目标维中选择单个强化属性或收集链接。 以下示例详细介绍了每种类型：
...
* **[!UICONTROL N个基数集合链接]**：主集中的每个记录都与链接数据中的0、1个或多个(N)条记录相关联。
...
1. 确定您要用来创建协调链接的数据。
   ...
   * 若要使用Campaign数据库中的数据创建协调链接，请选择&#x200B;**数据库架构**&#x200B;并选择存储目标的架构。
   * 若要使用来自输入转换的数据创建协调链接，请选择&#x200B;**临时架构**&#x200B;并选择存储目标数据的工作流转换。

1. 根据所选目标架构自动填充&#x200B;**标签**&#x200B;和&#x200B;**名称**&#x200B;字段。 如有必要，可以更改它们的值。

1. 在&#x200B;**协调条件**&#x200B;部分中，指定您希望如何协调源表和目标表中的数据：
...
1. 在&#x200B;**[!UICONTROL 扩充]**&#x200B;活动的&#x200B;**[!UICONTROL 优惠建议]**&#x200B;部分，单击&#x200B;**[!UICONTROL 添加优惠]**&#x200B;按钮
...
### 使用Enflication活动提供的优惠

在工作流中，如果要使用从交货中的强化活动获得的聘用，请执行以下步骤：

1. 打开交付活动并进入内容版本。 单击&#x200B;**[!UICONTROL 优惠设置]**&#x200B;按钮，然后在下拉列表中选择与您的优惠对应的&#x200B;**[!UICONTROL 优惠空间]**。
如果只想查看强化活动中的聘用，请将**[!UICONTROL 建议]**&#x200B;的数量设置为0，然后保存所做的修改。
