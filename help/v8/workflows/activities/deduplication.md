...
>title="用于识别重复项的字段"
>abstract="在 **用于标识重复项的字段** 部分，单击 **添加属性** 按钮来指定允许为其标识重复项的字段，例如：电子邮件地址、名字、姓氏等。 利用字段的顺序，可指定要先处理的字段。"
...
1. 在 **用于标识重复项的字段** 部分，单击 **添加属性** 按钮来指定允许为其标识重复项的字段，例如：电子邮件地址、名字、姓氏等。 利用字段的顺序，可指定要先处理的字段。
