...
1. 表达式编辑器将打开。 Adobe Campaign数据库中可用的个性化字段在屏幕左侧被整理到多个菜单中：
   ...
   | ![收件人菜单图标](assets/do-not-localize/perso-recipients-menu.png){zoomable="yes"} | **[!UICONTROL 收件人]**&#x200B;菜单列出了在收件人表中定义的字段，如收件人的姓名、年龄或地址。 当从外部文件[上载独立电子邮件投放的联系人时](../audience/file-audience.md)，此菜单列出输入文件中所有可用的字段。 |
